  ...
  <main class="workbench">
    ...
    <template v-else>
      ...
      <n-card v-if="selectedSession" class="data-panel" :bordered="false" style="margin-top:16px">
        ...
        <dl v-else class="status-list">
          <div v-for="t in selectedSession.tasks" :key="t.task_id">
            <dt>{{ t.task_id }}<small v-if="t.run_id" style="color:#9ca3af"> · {{ t.run_id }}</small></dt>
      ...
      <n-card class="data-panel" :bordered="false" style="margin-top:16px">
        <template #header><div><span class="panel-kicker">FLOW</span><h2>流程轨迹 <small style="color:#9ca3af">{{ trajectoryEvents.length }} 步</small></h2></div></template>
      ...
      <n-card v-if="selectedSession" class="data-panel" :bordered="false" style="margin-top:16px">
        <template #header><div><span class="panel-kicker">GUIDANCE</span><h2>追加指导 / 继续深挖 <small style="color:#9ca3af">(提交=意图,走确认门后续跑,不自动执行)</small></h2></div></template>
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-top:8px;flex-wrap:wrap">
          ...
          <span v-if="guidanceErr" style="color:#dc2626;font-size:13px">{{ guidanceErr }}</span>
          <span v-if="guidanceOk" style="color:#159c84;font-size:13px">{{ guidanceOk }}</span>
          <span v-if="forkMsg" style="color:#159c84;font-size:13px">{{ forkMsg }}</span>
        </div>
