...
<template>
  <main class="workbench">
    <section class="page-heading">
      <div><h1>会话 / 轨迹</h1><p>{{ projectDetail?.target || '从「项目」页选择一个项目' }}</p></div>
    ...
    <template v-else>
      <n-card class="data-panel" :bordered="false">
        <template #header><div><h2>会话</h2></div></template>
      ...
      <n-card v-if="selectedSession" class="data-panel" :bordered="false" style="margin-top:16px">
        <template #header><div><h2>谁在跑</h2></div></template>
      ...
      <n-card class="data-panel" :bordered="false" style="margin-top:16px">
        <template #header><div><h2>流程轨迹 <small style="color:var(--pa-text-3)">{{ trajectoryEvents.length }} 步</small></h2></div></template>
      ...
      <n-card v-if="selectedSession" class="data-panel" :bordered="false" style="margin-top:16px">
        <template #header><div><h2>追加指导 / 继续深挖 <small style="color:var(--pa-text-3)">(提交=意图,走确认门后续跑,不自动执行)</small></h2></div></template>
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-top:8px;flex-wrap:wrap">
          ...
          <n-button size="small" ghost :disabled="!projectDetail" @click="forkSession">复刻续跑(同目标,新建一份,仍走确认门)</n-button>
