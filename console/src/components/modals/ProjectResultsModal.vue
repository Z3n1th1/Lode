  ...
  <n-modal v-model:show="projectResultsModal" preset="card" :title="'成果 · ' + (projectResults?.target || '')" :style="{ width: '92vw', maxWidth: '760px' }" :bordered="false">
    ...
    <template v-else-if="projectResults">
      ...
      <template v-if="projectResults.ptt && (projectResults.ptt.top_leaves || []).length">
        ...
        <p style="color:var(--pa-text-3);font-size:12px;margin:0 0 8px">层级任务树(资产→功能点→假设→验证步),每轮开跑复诵这些高价值未完成叶子抗上下文漂移。</p>
        <div class="pending-list">
          <article v-for="(l, i) in projectResults.ptt.top_leaves" :key="i" class="pending-row" style="flex-direction:row;align-items:center;gap:8px">
            ...
            <span style="color:var(--pa-warning);min-width:44px">价值 {{ l.value }}</span>
            <strong>{{ l.label }}</strong>
            <small style="color:var(--pa-text-3)">{{ l.kind }}</small>
          </article>
        </div>
      </template>
      ...
      <template v-if="projectResults.attack_graph && (projectResults.attack_graph.chains || []).length">
        ...
        <p style="color:var(--pa-text-3);font-size:12px;margin:0 0 8px">provides/requires 可达性检索出的候选链假设;闭合的危险链深利用前必须人工门 + Verifier 二次,本层不自动执行。</p>
        <div class="pending-list">
          <article v-for="(c, i) in projectResults.attack_graph.chains" :key="i" class="pending-row">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
              ...
              <span style="color:var(--pa-warning)">价值 {{ c.value }}</span>
            ...
            <span v-if="c.satisfied_by.length" style="color:var(--pa-success)">已凑齐: {{ c.satisfied_by.join('、') }}</span>
            <span v-if="c.missing.length" style="color:var(--pa-text-3)">还差: {{ c.missing.join('、') }}</span>
            <small>{{ c.note }}</small>
          </article>
        </div>
      </template>
