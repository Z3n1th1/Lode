  ...
  return (
    <section className="shrink-0 border-t border-line">
      <h2 className="flex items-baseline gap-3 border-b border-line bg-raised/60 px-4 py-1.5">
        <span className="font-mono text-2xs tracking-wide text-fg-3">会话与轨迹</span>
        <span className="font-mono text-2xs text-fg-4">{detail.sessions.length}</span>
      </h2>
      ...
      {detail.sessions.length ? (
          ...
          {detail.sessions.map((session) => {
            ...
            return (
              <li key={session.session_id}>
                ...
                >
                  <span className="font-mono text-xs text-fg">{session.session_id}</span>
                  ...
                  <span className="font-mono text-xs text-fg-4">{session.profile_name}</span>
        ...
        <p className="px-4 py-6 text-center text-sm text-fg-3">该项目暂无会话</p>
      )}
      ...
      {selectedSessionId ? (
        <>
          <ul>
            {trajectory.length ? (
              trajectory.map((event) => (
                ...
                >
                  <span className="text-right font-mono text-2xs text-fg-4 tabular-nums">
                    {event.seq}
                  </span>
                  <span className="font-mono text-xs text-fg-4">
                    {TRAJ_KIND_LABEL[event.kind] ?? event.kind}
                  </span>
                  <span className="flex flex-wrap items-baseline gap-x-2 text-sm text-fg-2">
              ...
              <li className="border-b border-line/60 px-4 py-6 text-center text-sm text-fg-3">
                该会话暂无可读轨迹
              </li>
            )}
          </ul>
          ...
          <div className="border-b border-line px-4 py-3">
            <p className="font-mono text-2xs text-fg-4">续跑指导</p>
            <div className="mt-2 flex items-start gap-2">
              <textarea
                ...
                className="min-h-9 flex-1 resize-none rounded-md border border-line bg-transparent px-2.5 py-1.5 text-sm text-fg transition-colors placeholder:text-fg-4 hover:border-line-strong focus:border-accent focus:outline-none"
              />
              <Button
                variant="outline"
                size="sm"
              ...
              <ul className="mt-1 border-t border-line/60">
                {guidanceRows.map((row) => (
                  <li key={row.id} className="border-b border-line/60 py-1.5">
                    <p className="font-mono text-2xs text-fg-4">
                      {row.id} · {row.status} · {formatTimestamp(row.created_at)}
                    </p>
                    <p className="mt-0.5 text-sm text-fg-2">{row.guidance}</p>
                  </li>
                ))}
              </ul>
            ) : null}
