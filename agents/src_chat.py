...
# 最后兜底:那个技能包读不到时才用它。这里**刻意不再放一份 doctrine 副本** ——
# 那份内容属于技能包(SKILL.md),在这里再抄一份只会两边慢慢漂开,而且没有任何东西
# 会提醒你已经不一致了。兜底只需要说明"没拿到作业规范",而不是假装自己就是规范。
SRC_SYSTEM_PROMPT = """你是 Lode。这次没有加载到作业规范(skill pack 不可用)。

只做一件事:基于已有上下文给出当前最有用的回复。不要假设自己拥有未被加载的能力、
知识或规则,也不要把通用常识当成这个产品的规范来执行。
"""
