...
def list_sessions(state_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    for sess_dir in chat_dir.iterdir():
        ...
        if bb_path.is_file():
            try:
                bb_doc = json.loads(bb_path.read_text(encoding="utf-8"))
                intents = bb_doc.get("intents") or []
                info["facts"] = len(bb_doc.get("facts") or [])
                info["intents_queued"] = len([i for i in intents if i.get("status") == "queued"])
                info["intents_done"] = len([i for i in intents if i.get("status") in ("completed", "dead_end", "blocked")])
                info["hints"] = len(bb_doc.get("hints") or [])
                # Collect targets
                targets = {i.get("target", "") for i in intents if i.get("target")}
                info["targets"] = sorted(targets)[:10]
            except (OSError, ValueError, TypeError):
                pass
        # Display name: explicit title → target domain → first user prompt → id
        if not title and info.get("targets"):
            title = _domain_of(info["targets"][0])
        if not title and info.get("last_user"):
